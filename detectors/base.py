...
class Detector():
    ...
    @property
    def _trained_base(self) -> str:
        """Package-local ``trained/`` root, *ignoring* any ``DETECTOR_TRAINED_SUBDIR``
        override (see ``trained_dir``).

        Home for fold-independent scratch that depends only on the input mesh --
        e.g. a render cache -- so it is shared across leave-one-out folds and the
        standard run instead of being regenerated under each fold's isolated model
        dir. Not auto-created (the writer makes its leaf).
        """
        return os.path.join(self._pkg_dir, "trained")
    ...
    def trained_dir(self) -> str:
        ...
        When the ``DETECTOR_TRAINED_SUBDIR`` environment variable is set, the
        learned state is isolated in that subfolder of ``trained/`` rather than
        overwriting the standard model. The leave-one-out regime
        (``evaluate.py --loo``) sets it to ``loo/<held-out adversary>`` so each fold
        trains a separate model and a later plain ``evaluate.py`` still finds the
        full-data one. The env var is the channel because the per-shape eval worker
        is a fresh ``spawn`` that rebuilds the detector with ``cls()`` + ``load()``
        and so inherits it but not instance state. It stays git-ignored either way
        (``detectors/*/trained/`` is ignored wholesale).
        """
        sub = os.environ.get("DETECTOR_TRAINED_SUBDIR")
        d = os.path.join(self._trained_base, sub) if sub else self._trained_base
    ...
    def eval_method(self, output_path: str, data_root: str = "data",
                    ...
                    train_split: "Split" = None, test_split: "Split" = None,
                    eval_split: "Split" = None, trained_subdir: str = None):
        ...
        ``train_split``/``test_split``/``eval_split`` override the splits loaded
        from ``data_root`` when given (each a ``Split`` of .obj paths). The
        leave-one-out evaluator uses this to fit on every adversary but one and
        score only the held-out one without materializing per-fold data folders.
        ``trained_subdir`` (e.g. ``loo/<held-out adversary>``) isolates this run's
        learned state under ``trained/`` via ``DETECTOR_TRAINED_SUBDIR`` so it does
        not overwrite the standard model; the env var is set for the duration of
        the call (and so inherited by the spawned eval workers) and restored after.
        ...
        """
        prev_subdir = os.environ.get("DETECTOR_TRAINED_SUBDIR")
        if trained_subdir is not None:
            os.environ["DETECTOR_TRAINED_SUBDIR"] = trained_subdir
        try:
            return self._eval_method_inner(
                output_path, data_root, train, install, timeout,
                train_split, test_split, eval_split)
        finally:
            if trained_subdir is not None:
                if prev_subdir is None:
                    os.environ.pop("DETECTOR_TRAINED_SUBDIR", None)
                else:
                    os.environ["DETECTOR_TRAINED_SUBDIR"] = prev_subdir

    def _eval_method_inner(self, output_path, data_root, train, install, timeout,
                           train_split, test_split, eval_split):
        ...
        if train:
            t0 = time.perf_counter()
            tr = train_split if train_split is not None else load_split(data_root, "train")
            te = test_split if test_split is not None else load_split(data_root, "test")
            self.train(tr, te)
            self.save()
